...
than by convention: each of the three decision points begins with an
`if tokenRequest.AccessingOUID == "" { … }` guard.
...
sequenceDiagram
    ...
    Note over CA: (A) CLIENT RESOLUTION - OU-independent
    CA->>CA: GetOAuthClientByClientID(client_id)<br/>verify secret
    Note over CA: resolves the app globally.<br/>Not scoped by any OU: the accessing OU<br/>has not been read yet.
    end

    CA->>+TH: request + authenticated client
    TH->>TH: AccessingOUID = r.PathValue("ouId")
    TH->>+TS: ProcessTokenRequest(tokenRequest, oauthApp)
    TS->>+CC: ValidateGrant

    rect rgb(255, 243, 224)
    Note over CC,Share: (B) ACCESSING-OU ADMISSION
    CC->>CC: accessing OU empty? -> allow, done
    CC->>CC: resolve the OU exists
    CC->>CC: oauthApp.OUID == accessing? -> owner, allow
    CC->>+Share: IsShared(application, appID, accessingOU)
    Share-->>-CC: granted?
    Note over CC: not granted -> 400 unauthorized_client
    end
...
|---|---|---|---|
| A | Client resolution | none | `clientauth.go:123` - `GetOAuthClientByClientID` |
| B | Application admission | **accessing** | `client_credentials.go:76` - `validateAccessingOU` |
| C | Scope filtering | **accessing** | `client_credentials.go:159` - `DownscopeToResourceServer(… AccessingOUID)` |
| D | Role and group resolution | **app owning** | `client_credentials.go:185` - `EvaluateAccessBatch(… oauthApp.OUID)` |
| E | Claims | **accessing** | `client_credentials.go:204` - `BuildClientAttributes(… AccessingOUID)` |

Points B, C and E are the only new decisions. D is untouched: it passes `oauthApp.OUID` exactly as it
...
### 8.1 The accessing OU rides on the token request, not the context

`AccessingOUID` is a field on `model.TokenRequest`
(`backend/internal/oauth/oauth2/model/token.go:30`), populated by the token handler from
`r.PathValue` (`handler.go:104`). Both patterns register the identical middleware chain
(`token/init.go:74-75`); the OU-scoped one differs only in that the path value is present.

The alternative was a context value, following `dpop.GetJkt(ctx)`. It was rejected because nothing
outside the OAuth2 layer needs the value: the handler that reads the path already builds the request
object the grant handler receives. A context value would have made the dependency invisible at every
call site and testable only through plumbing.

### 8.2 Client resolution stays OU-independent

Point A resolves the application globally, before the accessing OU is read. This is deliberate and is
what makes one credential pair work everywhere: if the client lookup were scoped by the accessing OU,
an application would have to *exist* in every organization it serves, which is the per-customer
registration the feature exists to avoid.

Admission is therefore a separate step (B) rather than a filter on the lookup. The application is
found, authenticated, and only then asked "may you be used here".

### 8.3 The accessing OU is resolved before its grant is consulted

`validateAccessingOU` resolves the organization unit through `ouService` *first*, then checks the
grant. That ordering is load-bearing rather than defensive: `allOus` is a blanket scope that matches
every organization unit without consulting the tree, so a grant check alone cannot reject an id that
names nothing. Before the existence check was added, an unknown id under an `allOus` grant passed
admission and then failed deep inside claim resolution as a 500.
...
| Scope the client lookup by the accessing OU | Rejected as a *replacement* for the global lookup: the application would have to exist in every organization it serves, which makes "one credential pair" impossible. Option 2's A' (§5) is a weaker version that keeps one registration and only restricts *visibility* to the accessing OU; that version is viable, but as §5 shows it decomposes into the existing A plus B unless `client_id` uniqueness also becomes per-organization-unit. |
...
- **Only `client_credentials` honours the prefix.** The route accepts any grant type, but
  `AccessingOUID` is read only by the client credentials handler. Another grant reaching
  `/ou/{ouId}/oauth2/token` is issued exactly as it would be on the bare endpoint, silently ignoring
  the organization unit in the path. Either the other handlers should honour it or the route should
  reject non-`client_credentials` grants.
