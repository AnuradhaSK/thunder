...
func registerRoutes(
	...
	// The accessing-OU middleware wraps client authentication rather than the other way round:
	// resolving the client is already scoped to the organization unit, so that unit has to be
	// settled first.
	handler := accessingOUMiddleware(ouService)(
		clientAuthMiddleware(http.HandlerFunc(tokenHandler.HandleTokenRequest)))

	// Both patterns run the identical chain. The /ou/{ouId} variant only differs in that the
	// organization unit is resolved and put on the context; issuance on the bare path is unchanged.
