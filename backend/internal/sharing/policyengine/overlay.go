...
func (c Containment) intersectPair(a, b Rule) Rule {
	...
	// Narrow applies when it resolves a rule. The clamp above has already done the intersecting, so
	// all that is left here is the case where the pinned rule brought no value of its own.
	if !out.Editable && out.AllowedValues != nil {
		if out.Value == nil {
			pinned := append([]string{}, *out.AllowedValues...)
			out.Value = &pinned
