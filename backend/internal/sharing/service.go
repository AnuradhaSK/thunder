...
// sharingService orchestrates policy storage, chain resolution and the resource type's own hooks. Every
...
// RegisterResourceType adds a resource type's declaration to the framework.
func (s *sharingService) RegisterResourceType(decl ResourceTypeDeclaration) {
...
func (s *sharingService) validateMembers(
	...
	if err != nil {
		s.logger.Error(ctx, "Resource type refused the members named by an overlay rule",
			log.String("resourceType", string(rt)), log.String("resourceID", resourceID),
...
func (s *sharingService) cleanUpLostVisibility(ctx context.Context, changed Policy) error {
...
// governed.
func (s *sharingService) candidatesFrom(ctx context.Context, p Policy) ([]lostCandidate, error) {
...
// twice.
func (s *sharingService) withDeclared(
...
func (s *sharingService) declaredForResource(rt ResourceType, resourceID string) []Policy {
	if s.declarativeStore == nil {
		return nil
	}
...
func (s *sharingService) findPolicy(
