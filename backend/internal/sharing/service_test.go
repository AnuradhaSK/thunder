...
func newMockStore(t interface {
	...
	e.ListStoredPolicyIDsForResource(a, a, a).RunAndReturn(st.ListStoredPolicyIDsForResource).Maybe()
...
func (f *storeState) ListStoredPolicyIDsForResource(
	_ context.Context, rt ResourceType, resourceID string,
) ([]string, error) {
...
func (s *ServiceTestSuite) SetupTest() {
	...
	s.svc = newSharingService(storeMock, nil, resolver, resolver, &fakeTransactioner{}, nil, nil, false)
...
func (s *ServiceTestSuite) declarative() SharingServiceInterface {
	resolver := testResolver()
	svc := newSharingService(s.store, newDeclarativePolicyStore(), resolver, resolver,
...
func (s *ServiceTestSuite) TestDeclarativePolicyMaterializesOnFirstEdit() {
	...
	resolver := testResolver()
	svc := newSharingService(s.store, declStore, resolver, resolver, &fakeTransactioner{}, nil, nil, false)
	svc.RegisterResourceType(s.decl)
...
func (s *ServiceTestSuite) TestCreateAllowsCrossTreeReachWhenConfigured() {
	permissive := newSharingService(mustMockStore(s.T()), nil, testResolver(), testResolver(),
		&fakeTransactioner{}, nil, nil, true)
...
func (s *ServiceTestSuite) TestCreateDeclarativeRejectsInvalidTargets() {
	declSvc := newSharingService(mustMockStore(s.T()), newDeclarativePolicyStore(), testResolver(), testResolver(),
		&fakeTransactioner{}, nil, nil, false)
...
func (s *ServiceTestSuite) TestExportKeepsBothPoliciesWhenIDsCollide() {
	declStore := newDeclarativePolicyStore()
	svc := newSharingService(mustMockStore(s.T()), declStore, testResolver(), testResolver(),
		&fakeTransactioner{}, nil, nil, false)
...
func (s *ServiceTestSuite) TestOwnerVisibilityIsNeverCached() {
	...
	svc := newSharingService(mustMockStore(s.T()), nil, testResolver(), testResolver(),
		&fakeTransactioner{}, visibility, nil, false)
	svc.RegisterResourceType(&testDeclaration{})
...
func (s *ServiceTestSuite) TestOwnerIsAnsweredOverAStaleCachedDenial() {
	...
	svc := newSharingService(mustMockStore(s.T()), nil, testResolver(), testResolver(), &fakeTransactioner{},
...
func (s *ServiceTestSuite) TestWritesDoNotConsultTheOverlayCache() {
	...
	resolver := testResolver()
	svc := newSharingService(s.store, nil, resolver, resolver, &fakeTransactioner{}, nil, overlay, false)
	svc.RegisterResourceType(&testDeclaration{})
	ctx := context.Background()
...
func (s *ServiceTestSuite) TestOverlayCleanupIsOverridablePerResourceType() {
	...
	resolver := testResolver()
	svc := newSharingService(storeMock, nil, resolver, resolver, &fakeTransactioner{}, nil, nil, false)
	svc.RegisterResourceType(decl)
...
func (s *ServiceTestSuite) TestResolveOverlayRulesNeverServesTheOwnerAStaleDenial() {
	...
	resolver := testResolver()
	overlay := newFakeOverlayRuleCache()
	svc := newSharingService(mustMockStore(s.T()), nil, resolver, resolver, &fakeTransactioner{}, nil, overlay, false)
	svc.RegisterResourceType(decl)
...
func (s *ServiceTestSuite) TestMemberRefusalRevealsNothingAboutTheMember() {
	decl := &memberValidatingDeclaration{refuse: map[string]error{
		"ghost":  errors.New("no permission with that name exists"),
		"hidden": errors.New("permission exists but belongs to another organization unit"),
	}}
	resolver := testResolver()
	svc := newSharingService(mustMockStore(s.T()), newDeclarativePolicyStore(), resolver, resolver,
		&fakeTransactioner{}, nil, nil, false)
	svc.RegisterResourceType(decl)
