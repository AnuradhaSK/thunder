...
type sharingPolicyStoreInterface interface {
	...
	// ListStoredPolicyIDsForResource returns the ids of the policies written to the database for
	// one resource, without hydrating them. An empty resourceID spans every resource of the type,
	// which is what the reverse lookup needs.
	ListStoredPolicyIDsForResource(ctx context.Context, rt ResourceType, resourceID string) ([]string, error)
...
func (s *sharingStore) ListPoliciesRelevantToChain(
	...
	query, args := buildRelevantPoliciesQuery(rt, resourceID, chainOUIDs, s.deploymentID)
...
// ListStoredPolicyIDsForResource returns the ids of the policies written to the database for one
// resource, without hydrating them.
func (s *sharingStore) ListStoredPolicyIDsForResource(
	ctx context.Context, rt ResourceType, resourceID string,
) ([]string, error) {
	dbClient, err := s.client()
	if err != nil {
	...
	query, args := queryListPolicyIDsForResource, []interface{}{string(rt), resourceID, s.deploymentID}
	if resourceID == "" {
		query, args = queryListPolicyIDsForType, []interface{}{string(rt), s.deploymentID}
	}
	rows, err := dbClient.QueryContext(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("failed to list stored sharing policy ids: %w", err)
	}
	out := make([]string, 0, len(rows))
	for _, row := range rows {
		out = append(out, stringField(row["id"]))
	}
	return out, nil
}
...
func (s *sharingStore) DeleteOverlayValuesForOU(
	...
	if _, err := dbClient.ExecuteContext(ctx, queryDeleteOverlayValuesForOU,
		string(rt), resourceID, ouID, s.deploymentID); err != nil {
