...
Deleting a policy also deletes the policies its sharees wrote, by the cascade on `PARENT_POLICY_ID`, and that is the intended depth rather than collateral damage. A policy's parent is the one policy covering its issuer, because only a frontier OU may write one. If the parent goes, its issuer can no longer see the resource, so nothing it granted onward can stand. Cleanup follows the same path: an OU that loses sight of the resource puts everything its own policies reached back on the queue, so the OUs below a cascaded policy are cleaned rather than silently abandoned.
...
| `PolicyHooks` | Cleans up the type's own per-OU state for the whole resource when visibility is lost |
