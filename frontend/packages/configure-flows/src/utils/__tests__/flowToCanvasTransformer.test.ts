...
describe('flowToCanvasTransformer', () => {
  ...
  describe('transformFlowToCanvas', () => {
    ...
    describe('Component Transformation', () => {
      ...
      it('should derive telephone and number input types from their variant', () => {
        const flowData = createBaseFlowData([
          {
            id: 'prompt-node',
            type: 'PROMPT',
            meta: {
              components: [
                {id: 'phone-input', type: 'PHONE_INPUT', variant: 'TELEPHONE'},
                {id: 'number-input', type: 'NUMBER_INPUT', variant: 'NUMBER'},
              ],
            },
            layout: {position: {x: 0, y: 0}, size: {width: 300, height: 200}},
          },
        ...
        const components = result.nodes[0].data.components as Record<string, unknown>[] | undefined;
        expect(components?.[0]?.['inputType']).toBe('tel');
        expect(components?.[1]?.['inputType']).toBe('number');
      });
